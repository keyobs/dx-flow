...
if (branch === "main" || branch === "master") {
  console.log(`🔒 Don't commit to ${branch}.`);
  process.exit(1);
}
...
const hasEnvFile = stagedFiles.some((file) => file === ".env" || /^\.env\./.test(file));
if (hasEnvFile) {
  console.log("❌ Error: You are attempting to commit a .env file!");
  process.exit(1);
}
...
const skipTests =
  ...
  git(["config", "hooks.skiptests"]).stdout.trim() === "1";

if (skipTests) {
  console.log("⏭️  Skipping tests.");
} else {
  console.log("🧪 Tests for staged files...");
  const stagedJs = stagedFiles.filter((file) => /\.(js|jsx|ts|tsx)$/.test(file));
